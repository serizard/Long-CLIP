...
class CLIP(nn.Module):
    ...
    def forward(self, image, text_long,text_short,rank):
        ...
        image_feat_all_long = torch.cat(distnn.all_gather(image_features_long), dim=0)#gather with grad
        image_features_all_short = torch.cat(distnn.all_gather(image_features_short), dim=0)
        text_feat_all_long = torch.cat(distnn.all_gather(text_features_long), dim=0)
        text_feat_all_short = torch.cat(distnn.all_gather(text_features_short), dim=0)
